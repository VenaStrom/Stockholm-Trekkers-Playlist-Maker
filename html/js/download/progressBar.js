...
const updateInfoBox = async () => {
    const status = await download.get();
    const currentFile = status.currentFile;

    if (status.status === "") {
        return;
    } else if (status.status === "progressing") {
        topStatusText.textContent = `Downloading video files, please wait...`;
        ...
        bottomStatusText.textContent = `Downloading ${receivedMB} / ${totalMB} MB (${currentFile.percent}%)`;
